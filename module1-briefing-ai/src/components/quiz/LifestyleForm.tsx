...
import type { FamilyMember, LifestyleHabits, BudgetRange } from '@/lib/types'
  ...
  function updateMemberCount(count: number) {
    ...
    if (count > current.length) {
      while (current.length < count) {
        current.push({ role: 'adult', age_range: '25–45' })
      }
    } else {
    ...
    >
      ...
      <section>
        ...
        <div className="flex items-center gap-4 mb-4">
          ...
          <div className="flex items-center gap-2">
            <button
              type="button"
              onClick={() => updateMemberCount(Math.max(1, familySize - 1))}
              className="w-8 h-8 rounded-full bg-stone-100 hover:bg-stone-200 font-bold text-stone-700 transition-colors"
            >
              −
            </button>
            ...
            <button
              type="button"
              onClick={() => updateMemberCount(Math.min(10, familySize + 1))}
              className="w-8 h-8 rounded-full bg-stone-100 hover:bg-stone-200 font-bold text-stone-700 transition-colors"
            >
              +
            </button>
          ...
          {members.map((m, i) => (
            <div key={i} className="flex gap-2 bg-stone-50 rounded-xl p-3">
              <select
                ...
                className="flex-1 text-sm bg-white border border-stone-200 rounded-lg px-2 py-1.5 text-stone-700"
              >
              ...
              <select
                value={m.age_range}
                onChange={(e) => updateMember(i, 'age_range', e.target.value)}
                className="flex-1 text-sm bg-white border border-stone-200 rounded-lg px-2 py-1.5 text-stone-700"
              >
      ...
      <section>
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 gap-3">
          {BUDGET_OPTIONS.map((opt) => (
            ...
            >
              <div className={`font-semibold ${budget === opt.value ? 'text-amber-700' : 'text-stone-800'}`}>
                {opt.label}
              </div>
      ...
      <section>
        ...
        <textarea
          ...
          placeholder="Ví dụ: Nhà có bà nội đi lại khó khăn. Cần phòng riêng để thờ cúng. Hay ô tô để ngay trong nhà..."
          rows={4}
      ...
      <button
        type="submit"
        disabled={isLoading}
        className="w-full py-4 bg-amber-500 hover:bg-amber-600 disabled:bg-stone-300 text-white font-semibold rounded-xl text-lg transition-colors shadow-sm"
      >
        {isLoading ? (
