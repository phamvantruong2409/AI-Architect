...
import { BUDGET_LABELS, CONSTRAINT_TYPE_LABELS, CONSTRAINT_TYPE_COLORS, formatDate } from '@/lib/utils'
    ...
    >
      ...
      <div className="bg-stone-900 text-white rounded-2xl p-6">
        ...
        <div className="mt-4 flex items-center gap-3">
          <span className="px-3 py-1 bg-amber-500 text-white text-sm font-semibold rounded-full">
            {brief.dominant_style}
          </span>
          {budgetRange && (
            <span className="px-3 py-1 bg-stone-700 text-stone-200 text-sm rounded-full">
      ...
      <div className="bg-white border border-stone-200 rounded-2xl p-5">
        <h2 className="font-semibold text-stone-800 mb-4 flex items-center gap-2">
          <span>🎨</span> Phân tích phong cách
        </h2>
        <div className="space-y-3">
          {Object.entries(brief.style_breakdown)
            .sort(([, a], [, b]) => b - a)
            .map(([style, pct]) => (
              <div key={style}>
      ...
      <div className="bg-white border border-stone-200 rounded-2xl p-5">
        <h2 className="font-semibold text-stone-800 mb-4 flex items-center gap-2">
          <span>🎭</span> Bảng màu gợi ý
        </h2>
        <div className="flex gap-3 flex-wrap">
          {brief.color_palette.map((color, i) => (
            <div key={i} className="flex flex-col items-center gap-1.5">
              <div
                className="w-14 h-14 rounded-xl shadow-sm border border-stone-100"
      ...
      {brief.design_constraints.length > 0 && (
        <div className="bg-white border border-stone-200 rounded-2xl p-5">
          <h2 className="font-semibold text-stone-800 mb-4 flex items-center gap-2">
            <span>⚠️</span> Ràng buộc thiết kế (bóc tách tự động)
          </h2>
          <div className="space-y-3">
            {brief.design_constraints.map((c, i) => (
              <div key={i} className="flex gap-3 p-3 bg-stone-50 rounded-xl">
                <span
                  className={`text-xs font-semibold px-2 py-0.5 rounded-full whitespace-nowrap h-fit ${
                    CONSTRAINT_TYPE_COLORS[c.type] || 'bg-stone-200 text-stone-700'
                  }`}
                >
      ...
      {brief.space_requirements.length > 0 && (
        <div className="bg-white border border-stone-200 rounded-2xl p-5">
          <h2 className="font-semibold text-stone-800 mb-4 flex items-center gap-2">
            <span>🏠</span> Yêu cầu không gian
          </h2>
          <div className="space-y-2">
            {brief.space_requirements.map((s, i) => (
              <div key={i} className="flex items-start gap-3">
                <span
                  className={`text-xs px-2 py-0.5 rounded-full whitespace-nowrap font-medium mt-0.5 ${
