...
import type { QuizSession, DesignBrief } from './types'
...
Chỉ trả về JSON thuần túy.`
  ...
  // Làm sạch response phòng trường hợp Gemini vẫn thêm markdown
  const cleaned = text.replace(/^```json\n?/, '').replace(/\n?```$/, '').trim()

  return JSON.parse(cleaned)
}

export async function analyzeStyleImages(imageUrls: string[]): Promise<Record<string, string[]>> {
  const model = genAI.getGenerativeModel({ model: 'gemini-2.0-flash' })

  const prompt = `Phân tích các hình ảnh nội thất này và với MỖI hình ảnh, trả về JSON với key là index (0, 1, 2...)
và value là mảng tags mô tả: chất liệu chính, tông màu, phong cách, mức độ ánh sáng.
Ví dụ: {"0": ["gỗ sáng", "trắng", "Scandinavian", "ánh sáng tự nhiên nhiều"]}
Chỉ trả JSON thuần túy.`

  const imageParts = imageUrls.map((url) => ({
    inlineData: { mimeType: 'image/jpeg' as const, data: url },
  }))
