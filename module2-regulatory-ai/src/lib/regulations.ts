import type { BuildingType, ZoningType } from './types'

// ===== QCXDVN 01:2021/BXD — Khoảng lùi theo số tầng =====
...
// ===== Mật độ xây dựng tối đa (%) theo loại công trình =====
...
// ===== Chiều cao tối đa theo số tầng (m) — quy định chung =====
// Chiều cao tầng điển hình 3.3m + tum/sân thượng

export function getMaxHeightForFloors(floors: number): number {
  return floors * 3.6 + 3.5   // tầng điển hình 3.6m + tum 3.5m (quy định thông thường)
}

// ===== QCVN 06:2022/BXD — PCCC =====

export const PCCC_RULES = {
  corridor_width_min: 1.2,      // Chiều rộng hành lang tối thiểu (m)
  stair_width_min: 1.2,         // Chiều rộng cầu thang thoát nạn (m)
  max_distance_to_exit: 25,     // Khoảng cách từ phòng xa nhất đến cầu thang (m)
  exit_door_width_min: 0.9,     // Chiều rộng cửa thoát hiểm (m)
}

// ===== TCVN 4474 — Chiếu sáng tự nhiên =====

export const LIGHTING_RULES = {
  window_ratio_min: 12.5,  // Tỷ lệ tối thiểu diện tích cửa/diện tích sàn (%)
}
