...
import type { ChatMessage, MatchedChunk, DocumentCategory } from '@/lib/types'
...
function SourceCard({ sources }: { sources: MatchedChunk[] }) {
  ...
  return (
    <div className="mt-2 border border-stone-800 rounded-lg overflow-hidden text-xs">
      <button
        onClick={() => setOpen((o) => !o)}
        className="w-full flex items-center justify-between px-3 py-2 bg-stone-900 text-stone-500 hover:text-stone-300 transition-colors"
      >
      ...
      <div className={`max-w-[80%] ${isUser ? 'items-end' : 'items-start'} flex flex-col gap-1`}>
        <div
          className={`px-4 py-3 rounded-2xl text-sm leading-relaxed whitespace-pre-wrap ${
            isUser
              ? 'bg-stone-700 text-stone-100 rounded-br-sm'
              : 'bg-stone-900 text-stone-200 rounded-bl-sm border border-stone-800'
          }`}
        >
          {msg.content}
        </div>
...
export default function ChatInterface() {
  ...
  async function sendMessage() {
    ...
    try {
      ...
      toast.error(err instanceof Error ? err.message : 'Lỗi khi gửi tin nhắn')
      ...
      <div className="px-4 py-2 border-b border-stone-800 flex items-center gap-2">
        ...
        <div className="flex gap-1">
          {CATEGORIES.map((c) => (
            <button
              key={c.value}
              onClick={() => setCategory(c.value as DocumentCategory | '')}
        ...
        {messages.length === 0 && (
          <div className="h-full flex flex-col items-center justify-center text-center gap-3">
            ...
            <div className="flex flex-wrap gap-2 justify-center">
              {[
                'Nhà 5 tầng cần khoảng lùi bao nhiêu?',
                'Yêu cầu PCCC cho chung cư cao tầng?',
                'Diện tích tối thiểu phòng ngủ theo TCVN?',
              ].map((q) => (
                <button
                  key={q}
                  onClick={() => setInput(q)}
                  className="text-xs bg-stone-800 hover:bg-stone-700 text-stone-400 px-3 py-1.5 rounded-full transition-colors"
                >
                  {q}
                </button>
              ))}
      ...
      <div className="border-t border-stone-800 px-4 py-4">
        <div className="flex gap-2">
          ...
          <button
            ...
            className="bg-stone-200 hover:bg-white disabled:opacity-40 text-stone-900 p-3 rounded-xl transition-colors"
          >
            <Send size={16} />
          </button>
        </div>
