...
export default function DocumentList({ documents, onDeleted, onProcessed }: Props) {
  ...
  async function handleProcess(doc: Document) {
    ...
    try {
      const res = await fetch(`/api/process/${doc.id}`, { method: 'POST' })
      const json = await res.json()
      if (!res.ok) throw new Error(json.error)
      toast.success(`Đã lập chỉ mục ${json.chunk_count} đoạn`)
      onProcessed(doc.id)
    } catch (err) {
      ...
      {documents.map((doc) => {
        ...
        return (
          ...
          >
            ...
            <StatusIcon
              size={16}
              className={`shrink-0 ${statusCfg.color} ${doc.status === 'processing' || isProcessing ? 'animate-spin' : ''}`}
            />
            ...
            <div className="flex-1 min-w-0">
              ...
              <div className="flex items-center gap-2 mt-0.5">
                <span className={`text-xs px-1.5 py-0.5 rounded ${CATEGORY_COLORS[doc.category] ?? CATEGORY_COLORS.general}`}>
                  {doc.category}
                </span>
                {doc.status === 'ready' && (
                  <span className="text-xs text-stone-600">{doc.chunk_count} đoạn</span>
                )}
                {doc.error_message && (
                  <span className="text-xs text-red-500 truncate">{doc.error_message}</span>
                )}
              </div>
            </div>
            ...
            <div className="flex items-center gap-1 shrink-0">
              {(doc.status === 'pending' || doc.status === 'error') && (
                <button
                  ...
                  title="Lập chỉ mục"
                  className="p-1.5 text-stone-400 hover:text-amber-400 disabled:opacity-40 transition-colors"
                >
                  {isProcessing ? (
              ...
              <button
                ...
                disabled={isDeleting}
                title="Xóa"
                className="p-1.5 text-stone-600 hover:text-red-400 disabled:opacity-40 transition-colors"
              >
